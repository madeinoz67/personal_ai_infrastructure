...
async function main() {
  ...
  if (!networkExists) {
    cli.info(`Creating isolated network: ${NETWORK_NAME}`);
    const result = await containerManager.createNetwork(NETWORK_NAME, "172.28.0.0/16");
    if (result.success) {
  ...
  if (!await containerManager.containerExists(FALKORDB_CONTAINER)) {
    ...
    if (result.success) {
      ...
      cli.dim(`  Network: ${NETWORK_NAME} (internal only)`);
    } else {
  ...
  cli.dim(`  FalkorDB:     ${FALKORDB_CONTAINER} (internal network only)`);
  cli.dim(`  MCP Server:   ${MCP_CONTAINER} (exposed to host)`);
  cli.dim(`  Network:      ${NETWORK_NAME} (isolated bridge)`);
  cli.blank();
  cli.info("Access points:");
  cli.url("MCP Server", "http://localhost:8000/mcp/");
  cli.url("Health Check", "http://localhost:8000/health");
  ...
  cli.blank();
  cli.warning("Note: FalkorDB port 6379 is NOT exposed to host (network isolation)");
  cli.warning("      Access FalkorDB only through the MCP server or web UI");
  cli.blank();
}
